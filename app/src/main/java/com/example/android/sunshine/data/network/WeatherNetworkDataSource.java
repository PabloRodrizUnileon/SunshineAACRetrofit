...
import com.example.android.sunshine.data.network.sync.SunshineFirebaseJobService;
import com.example.android.sunshine.data.network.sync.SunshineSyncIntentService;
import com.example.android.sunshine.data.network.util.NetworkUtils;
...
public class WeatherNetworkDataSource {
    ...
    //  LiveData storing the latest downloaded forecasts
    private MutableLiveData<List<WeatherEntry>> mDownloadedWeatherForecasts;
    ...
    public MutableLiveData<List<WeatherEntry>> getCurrentWeatherForecasts() {
    ...
    public void fetchWeather() {
        ...
        mExecutors.networkIO().execute(() -> {
            try {
                ...
                Call<WeatherResponse> call = WeatherApi.getWeatherWebServiceInstance().getWeatherResponse();

                WeatherResponse weatherResponse = call.execute().body();
                List<WeatherEntry> weatherEntries = weatherResponse.getWeatherEntries();
                long normalizedUtcStartDay = SunshineDateUtils.getNormalizedUtcMsForToday();

                //  Asign to every entry of weatherEntry, a Date object, incremented;
                //  meaning that every WeatherEntry would have a date of day.
                for (int i = 0; i < weatherEntries.size(); i++) {
                    ...
                    weatherEntry.setDate(new Date(dateTimeMillis));
                }
                mDownloadedWeatherForecasts.postValue(weatherEntries);
//
